...
let clients = {}; // Store clients with their roles and rooms
...
wss.on('connection', ws => {
  ws.on('message', message => {
    ...
    switch (data.type) {
      case 'register':
        clients[data.userId] = { ws, role: data.role, rooms: [] };
        break;
      case 'joinRoom':
        const { userId, questionId } = data;
        if (clients[userId]) {
          clients[userId].rooms.push(questionId);
        }
        break;
      case 'message':
        console.log("message:", message);
        const { message, userId: senderId, questionId: roomId } = data;
        const sender = clients[senderId];
        if (sender && sender.role === 'expert') {
          for (let clientId in clients) {
            if (clients[clientId].rooms.includes(roomId)) {
          ...
          sender.ws.send(JSON.stringify({ type: 'error', message: 'Only experts can answer questions.' }));
        }
        break;
...
function broadcast(message, senderId = null) {
  for (let clientId in clients) {
    if (clients[clientId].ws.readyState === WebSocket.OPEN && clientId !== senderId) {
...
async function assignQuestionToExpert(questionText, subject) {
  ...
  // Notify all experts about the new question
  broadcast({ type: "new-question", question: newQuestion, subject });
  ...
  setTimeout(async () => {
    ...
    if (question && question.status === "pending") {
      ...
      if (nextExpert) {
        ...
        broadcast({ type: "reassigned-question", question, subject });
